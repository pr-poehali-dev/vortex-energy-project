...
CORS_HEADERS = {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, X-Admin-Token, X-Admin-Mode',
}
...
def is_admin_mode(event):
    return event.get('headers', {}).get('X-Admin-Mode', '') == 'true'
...
def handler(event: dict, context) -> dict:
    ...
    # GET без admin-режима — публичный список активных баннеров
    if method == 'GET' and not is_admin_mode(event):
        cur.execute(
    ...
    # Все admin-запросы требуют авторизации
